...
def call_gemini_ai(prompt, sys_prompt, image_blobs=None):
    ...
    try:
        ...
        if res.status_code == 200:
            add_log("AI: Analysis Successful."); return res.json()['candidates'][0]['content']['parts'][0]['text']
    except Exception as e: add_log(f"AI: Exception {str(e)}")
    return None
...
if st.session_state.page == 1:
    ...
    if pts >= 11 or st.session_state.mock_assets:
        if st.button("📝 GENERATE 15 MC ANALYSIS", width="stretch"):
            ...
            if res: st.session_state.mc_questions = json.loads(res.replace("```json", "").replace("```", "")); st.rerun()
        if st.session_state.mc_questions:
            for i, q in enumerate(st.session_state.mc_questions):
...
# ==========================================
    ...
    if not st.session_state.sync_complete:
        ...
        if st.session_state.generated_content is None:
            ...
            res = call_gemini_ai(ctx, "Output JSON object: {'BoringChallenge':'...', 'CreativeSolution':'...', 'SocialMedia':{'FB':'...', 'LI':'...', 'IG':'...', 'TR':'...'}, 'Web':{'EN':'...', 'TC':'...', 'JP':'...'}, 'FAQ':{'EN':[{'q':'','a':''}], 'TC':[], 'JP':[]}}")
