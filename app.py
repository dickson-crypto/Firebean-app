...
STABLE_MODEL_ID = "gemini-2.0-flash" # Optimized for vision + speed
...
def safe_flatten_faq(faq_input):
    ...
    if isinstance(faq_input, str):
        ...
        try:
            ...
            # dumps 預設沒有 indent，這會輸出完美的單行字串，完美解決 Google Sheet 錯位問題
            return json.dumps(parsed, ensure_ascii=False)
        except Exception as e:
            log_debug(f"FAQ 扁平化警告: {str(e)}", "warning")
            # 解析失敗的備用方案：直接取代所有換行符號，確保它是安全的單行字串
...
def init_session_state():
    fields = {
        ...
        "draft_project_id": "",  
        "loaded_image_urls": [],  
        # 統一使用 _edit 作為資料唯一來源，避免 Streamlit 狀態衝突
        "faq_en_edit": "",  
        "faq_tc_edit": "",  
        "faq_jp_edit": "",  
    }
...
"""
                    ...
                    try:
                        ...
                        if isinstance(data, dict):
                            ...
                            if isinstance(faq_data, dict):
                                ...
                                # 🚀 正確賦值給 _edit 變數
                                st.session_state.faq_en_edit = format_faq(faq_data.get("en", "[]"))
                                st.session_state.faq_tc_edit = format_faq(faq_data.get("tc", "[]"))
                                st.session_state.faq_jp_edit = format_faq(faq_data.get("jp", "[]"))
        ...
        if st.session_state.ai_content:
            ...
            if st.session_state.get("faq_en_edit") or st.session_state.get("faq_tc_edit") or st.session_state.get("faq_jp_edit"):
                st.markdown("---")
                ...
                with faq_tabs[0]:
                    st.session_state.faq_en_edit = st.text_area("FAQ EN", height=250, value=st.session_state.faq_en_edit, key="faq_en_ta", help='Format: [{"Q1": "...", "A1": "..."}, ...]')
                with faq_tabs[1]:
                    st.session_state.faq_tc_edit = st.text_area("FAQ TC", height=250, value=st.session_state.faq_tc_edit, key="faq_tc_ta", help='Format: [{"Q1": "...", "A1": "..."}, ...]')
                with faq_tabs[2]:
                    st.session_state.faq_jp_edit = st.text_area("FAQ JP", height=250, value=st.session_state.faq_jp_edit, key="faq_jp_ta", help='Format: [{"Q1": "...", "A1": "..."}, ...]')
